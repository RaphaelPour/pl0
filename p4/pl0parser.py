...
class Edge():
    ...
    def __str__(self):
        ...
        if self.type == EdgeType.SYMBOL___:
            if(isinstance(self.value, MorphemSymbol)):
...
class PL0Parser():
    ...
    def __init__(self, sourceFile):
        ...
        blockEdges = [
            ...
            Edge(EdgeType.SYMBOL___, MorphemSymbol.CONST,None, 1,6, BLCK), # 0
            ...
            Edge(EdgeType.SYMBOL___, MorphemSymbol.VAR, None, 7,10, BLCK), # 6
            ...
            Edge(EdgeType.SYMBOL___, MorphemSymbol.PROCEDURE, None, 11,15, BLCK), # 10
        ...
        statementEdges = [
            ...
            Edge(EdgeType.SYMBOL___, MorphemSymbol.ASSIGN, None, 2,0,STAT),    #  1
            Edge(EdgeType.SUBGRAPH_, EXPR, None, 21,0,STAT),                   #  2

            # Condition
            Edge(EdgeType.SYMBOL___,MorphemSymbol.IF, None, 4,7,STAT),         #  3
            Edge(EdgeType.SUBGRAPH_, COND, None, 5,0,STAT),                    #  4
            Edge(EdgeType.SYMBOL___, MorphemSymbol.THEN,None, 6,0,STAT),       #  5
            Edge(EdgeType.SUBGRAPH_, STAT,None, 21,0,STAT),                    #  6

            # While Loop
            Edge(EdgeType.SYMBOL___, MorphemSymbol.WHILE, None, 8, 11,STAT),   #  7
            Edge(EdgeType.SUBGRAPH_, COND, None, 9,0,STAT),                    #  8
            Edge(EdgeType.SYMBOL___, MorphemSymbol.DO,None, 10,0,STAT),        #  9
            Edge(EdgeType.SUBGRAPH_, STAT,None, 21,0,STAT),                    # 10

            # BEGIN - END
            Edge(EdgeType.SYMBOL___, MorphemSymbol.BEGIN, None, 12,15,STAT),   # 11
            Edge(EdgeType.SUBGRAPH_, STAT, None,3, 0,STAT),                    # 12
            Edge(EdgeType.SYMBOL___, ';', None, 12, 14,STAT),                  # 13
            Edge(EdgeType.SYMBOL___, MorphemSymbol.END,None, 21,0,STAT),       # 14

            # CALL
            Edge(EdgeType.SYMBOL___, MorphemSymbol.CALL,None, 16,17,STAT),     # 15
        ...
        conditionEdges = [
            # ODD
            Edge(EdgeType.SYMBOL___, MorphemSymbol.ODD, None, 1, 2,COND),   #  0
            ...
            Edge(EdgeType.SYMBOL___, MorphemSymbol.LESSER_EQUAL, None, 9, 8,COND),  #  7
            Edge(EdgeType.SYMBOL___, MorphemSymbol.GREATER_EQUAL, None, 9, 0,COND), #  8
    ...
    def parse(self, edge=None, path=[]):
        ...
        while(True):
            ...
            if(edge.type == EdgeType.SYMBOL___):
                ...
                if success:
                    path.append({ 'value' : self.lexer.morphem.value, 'type' : edge.type, 'line' : self.lexer.morphem.lineCount})
            elif(edge.type == EdgeType.MORPHEM__):
                success = self.lexer.morphem.code == edge.value
                if success:
                    path.append({ 'value' : self.lexer.morphem.value, 'type' : edge.type, 'line' : self.lexer.morphem.lineCount})
            elif(edge.type == EdgeType.SUBGRAPH_):
                path.append({ 'value' : edge.nonterminal.name, 'type' : edge.type, 'line' : self.lexer.morphem.lineCount})
